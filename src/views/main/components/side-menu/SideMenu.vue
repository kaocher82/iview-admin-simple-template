      ...
      <template v-for="item in menuList">
        ...
        <Tooltip transfer v-else
          ...
          <a @click="handleSelect(getNameOrHref(item, true))" class="drop-menu-a" :style="{textAlign: 'center'}">
            <Icon :size="rootIconSize" :color="theme === 'dark' ? '#fff' : '#495060'"
...
export default {
  ...
  watch: {
    activeName (name) {
      ...
      else this.openedNames = this.getIntersection(this.openedNames, this.getOpenedNamesByActiveName(name))
    },
    openNames (newNames) {
  ...
  mounted () {
    this.openedNames = this.getIntersection(this.openedNames, this.getOpenedNamesByActiveName(name))
  }
}
</script>
